...
const CategoryAdmin = () => {
  ...
  const [imageUpd, setImageUpd] = useState('');
  ...
  const getCategoryById = async (id) => {
    try {
      ...
      setNameUpd(response.data.data.name)
      setDescriptionUpd(response.data.data.description)
      setSortOrderUpd(response.data.data.sort_order)
      setImageUpd(response.data.data.image)
      console.table(response.data.data)
    } catch (error) {
  ...
  const updateCategory = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${baseUrl}categories/${selectedId}`, {
        name: nameUpd,
        description: descriptionUpd,
        sortOrder: sortOrderUpd,
        image_url: imageUpd
      }, {
        withCredentials: true
      })
